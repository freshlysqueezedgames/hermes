...
export default class Hermes {
  ...
  Update (steps: Array, action: Action) : Hermes {
    ...
    const store: Object = t.Branch(t.store, steps, (node : Object, step : string, i : number) : Object => {
      ...
      let state : Object | Array

      t.SetContext(path.join('/'), action.context, function () {
        return state
      })

      return (state = (!result || !result.reducer ? t.reducer : result.reducer).Reduce(action, node, payload))
    })
